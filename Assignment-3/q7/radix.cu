...
__device__ unsigned int ddata[WORDSIZE];
__device__ int ddata_s[WORDSIZE];
...
__global__ void parallelRadix()
{
	...
	__shared__ volatile unsigned int sdata[WORDSIZE * 2];
	...
	{
		unsigned int mydata = sdata[((WORDSIZE - 1) - threadIdx.x) + offset];
...
int main() {
	...
	for (int lcount = 0; lcount < LOOPS; lcount++)
	{
		...
		cudaMemcpyToSymbol(ddata, hdata, WORDSIZE * sizeof(unsigned int));

		// Execution time measurement, that point starts the clock
		high_resolution_clock::time_point t1 = high_resolution_clock::now();
		parallelRadix <<< 1, WORDSIZE >>>();
		...
		cudaMemcpyFromSymbol(hdata, ddata, WORDSIZE * sizeof(unsigned int));
	}

	printf("Parallel Radix Sort:\n");
	printf("Array size = %d\n", WORDSIZE * LOOPS);
	printf("Time elapsed = %fmilliseconds\n", totalTime);

	/* Serial Radix Sort */

	unsigned int hdata_s[WORDSIZE];
	totalTime = 0;
	...
	for (int lcount = 0; lcount < LOOPS; lcount++)
	{
		...
		// Copy data from host to device
		cudaMemcpyToSymbol(ddata_s, hdata_s, WORDSIZE * sizeof(unsigned int));
		...
		// Copy data from device to host
		cudaMemcpyFromSymbol(hdata_s, ddata_s, WORDSIZE * sizeof(unsigned int));
	}
